...
const EventCard: React.FC<EventCardProps> = ({ id, title, excerpt, image, slug, date }) => {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
      <div className="relative h-48 bg-cover bg-center" style={{ backgroundImage: `url(${image})` }}>
        <div className="absolute inset-0 bg-black/20"></div>
        <div className="absolute top-4 left-4 bg-[#F58220] text-white px-3 py-1 rounded-full text-sm font-semibold">
          {date}
        </div>
      </div>
